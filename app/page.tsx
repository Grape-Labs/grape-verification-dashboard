...
import { useRpcSettings } from "./providers";
...
function DiscordBrandIcon({
  ...
  return (
    <SvgIcon viewBox="0 0 40 40" sx={sx}>
      <path
        fill="currentColor"
        fillRule="evenodd"
        clipRule="evenodd"
...
function TelegramBrandIcon({
  ...
  return (
    <SvgIcon viewBox="0 0 32 32" sx={sx}>
      <path
        fill="currentColor"
        d="M7.034 15.252c4.975-2.167 8.293-3.596 9.953-4.287 4.74-1.971 5.725-2.314 6.366-2.325.142-.002.457.033.662.198.172.14.22.33.243.463.022.132.05.435.028.671-.257 2.7-1.368 9.248-1.933 12.27-.24 1.28-.71 1.708-1.167 1.75-.99.091-1.743-.655-2.703-1.284-1.502-.985-2.351-1.598-3.81-2.558-1.684-1.11-.592-1.721.368-2.718.252-.261 4.619-4.233 4.703-4.594.01-.045.02-.213-.08-.301-.1-.09-.246-.059-.353-.035-.15.034-2.55 1.62-7.198 4.758-.682.468-1.298.696-1.851.684-.61-.013-1.782-.344-2.653-.628-1.069-.347-1.918-.53-1.845-1.12.039-.308.462-.623 1.27-.944Z"
      />
    </SvgIcon>
  );
}
...
function platformIcon(platform: PlatformKey) {
  switch (platform) {
    case "discord":
      return <DiscordBrandIcon sx={{ fontSize: 18 }} />;
    case "telegram":
      return <TelegramBrandIcon sx={{ fontSize: 18 }} />;
    case "twitter":
...
function rpcHostForDisplay(endpoint: string) {
  try {
...
export default function Page() {
  ...
  const {
    ...
    defaultEndpoint: defaultRpcEndpoint,
    customEndpoint: customRpcEndpoint,
    setCustomEndpoint,
    resetEndpoint: resetRpcEndpoint,
  } = useRpcSettings();
  ...
  const [rpcTestOk, setRpcTestOk] = useState<boolean | null>(null);
  ...
  function validatedRpcUrl(value: string): string {
    ...
    if (!trimmed) throw new Error("Enter an RPC URL first.");
    ...
    if (parsed.protocol !== "https:" && !localHttp) {
      throw new Error("RPC URLs must use HTTPS (HTTP is allowed for localhost). ");
    }
    if (parsed.username || parsed.password) {
      throw new Error("RPC URLs cannot contain URL username/password credentials.");
    }
    return parsed.toString();
  }

  async function testRpc(applyAfterTest: boolean) {
    setRpcTesting(true);
    setRpcTestOk(null);
    setRpcTestResult("Testing connection...");

    try {
      const endpoint = validatedRpcUrl(rpcInput);
      const candidate = new Connection(endpoint, { commitment: "confirmed" });
      const startedAt = performance.now();
      const [genesisHash] = await Promise.all([
        candidate.getGenesisHash(),
        candidate.getLatestBlockhash("confirmed"),
      ]);
      const elapsed = Math.round(performance.now() - startedAt);

      if (applyAfterTest) setCustomEndpoint(endpoint);
      setRpcTestOk(true);
      setRpcTestResult(
        `Connected in ${elapsed} ms • genesis ${genesisHash.slice(0, 8)}…${
          applyAfterTest ? " • saved and applied" : ""
        }`
      );
    } catch (e: unknown) {
      setRpcTestOk(false);
  ...
  function restoreDefaultRpc() {
    ...
    setRpcInput("");
    setRpcTestOk(null);
    setRpcTestResult("Default browser RPC restored.");
  }
        ...
        {advancedMode && (
          <>
            ...
            {advancedOpen && (
              <>
                <Paper sx={{ p: 2.5, mt: 2.5 }}>
                  <Typography variant="h3">Browser RPC</Typography>
                  <Typography
                    variant="body2"
                    sx={{ mt: 0.5, opacity: 0.75, fontFamily: "system-ui" }}
                  >
                    Choose the Solana RPC used for account reads and
                    wallet-signed transactions on this device. Wallet linking
                    and unlinking continue to use the protected server attestor
                    RPC.
                  </Typography>
                  ...
                  <Stack spacing={1.25}>
                    <InfoRow
                      label="Active source"
                      value={customRpcEndpoint ? "Custom RPC" : "App default"}
                    />
                    <Typography
                      sx={{
                        fontFamily: "system-ui",
                    ...
                    >
                      Active host: {rpcHostForDisplay(rpcEndpoint)}
                    </Typography>
                    ...
                    <Box
                      ...
                      aria-label="Custom Solana RPC URL"
                      autoCapitalize="none"
                      autoCorrect="off"
                      spellCheck={false}
                      style={{
                    ...
                    >
                      ...
                      <Button
                        variant="contained"
                        onClick={() => void testRpc(true)}
                        disabled={rpcTesting || !rpcInput.trim()}
                      >
                        Test, save &amp; apply
                      </Button>
                      <Button
                        onClick={restoreDefaultRpc}
                        disabled={rpcTesting || !customRpcEndpoint}
                      >
                        Reset to default
                      </Button>
                    </Stack>
                    ...
                    <Typography
                      sx={{ fontFamily: "system-ui", fontSize: 11, opacity: 0.6 }}
                    >
                      Default host: {rpcHostForDisplay(defaultRpcEndpoint)}.
                      Custom URLs are stored only in this browser. URLs
                      containing provider API keys remain visible to browser
                      code.
                    </Typography>
                  </Stack>
                </Paper>
